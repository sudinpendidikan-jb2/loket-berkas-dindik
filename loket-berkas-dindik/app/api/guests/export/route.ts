...
export async function GET(req: NextRequest) {
  ...
  const guests = await listGuests({ date, instansi, status, q });
  ...
  const jenjangSlug = instansi ? `-${instansi.toLowerCase().replace(/\s+/g, "-")}` : "";
  const filename = `daftar-tamu-${date ?? "semua"}${jenjangSlug}.csv`;
